...
function App() {
  ...
  const deleteFile = id => {
    ...
    const { path, title, isNew } = files[id];
    if (isNew) {
  ...
  //删除七牛云的数据后删除本地数据
  const activeFileDeleted = (event, id) => {
    const { [id]: value, ...afterDelete } = files;
    setFiles(afterDelete);
    saveFilesToStore(afterDelete);
    tabClose(id);
  ...
  //重命名七牛云上的文件后
  const activeFileRenamed = (event, data) => {
    const { oldTitle, newTitle, id } = data;
    const oldPath = files[id].path;
    const newPath = join(dirname(files[id].path), `${newTitle}.md`);
    const modifiedFile = {
      ...files[id],
      title: newTitle,
      isNew: false,
  ...
  const activeFileAllDownloaded = (event, data) => {
    setFiles(data);
  };
  ...
  useIpcRenderer({
    ...
    "active-file-deleted": activeFileDeleted,
    "active-file-renamed": activeFileRenamed,
    "active-file-all-downloaded": activeFileAllDownloaded,
